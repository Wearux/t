...
public class task {
    ...
    public static  boolean IsPrime (int n)
    {
        ...
        for (int i = 2; i < n / 2; i++)
        {
            if((n % i==0)&&(n/i!=1.0)) {itog = true  ; break;}
        }
        return  itog;
    }
    ...
    {
        int buf;
        if ((x==1 && y==2)||(x==2 && y==1)) buf=3;
        else if((x==1 && y==3)||(x==3 && y==1)) buf=2;
        else buf=1;
        if(n>1){
            move(n-1,x,buf);
            System.out.format("Disk %d move from %d to %d %n",n,x,y);
            move(n-1,buf,y);
        }
        else System.out.format("Disk %d move from %d to %d %n",n,x,y);
    }
    public static void task26(Scanner sc){
        int n=sc.nextInt();
        System.out.format("мин. шагов: %.0f %n", powerOrSuperRecursia(2,n)-1);
        move(n,1,2);
    }
    ...
    public static int CountMax(int A[],int n){
        ...
        for (int i = 0; i < n/2; i++) {
            if(max==A[i]) maxcount++;
            else if(max<A[i]) {max=A[i]; maxcount=1;}
            if(max==A[n-1-i]) maxcount++;
            else if(max<A[n-1-i]) {max=A[n-1-i]; maxcount=1;}
    ...
    public static void LastMax(int A[]){
        ...
        for (int i = 0; i < n/2; i++) {
            if(max==A[i]);
            else if(max<A[i]) {max=A[i]; maxindex=i;}
            if(max==A[n-1-i] && maxindex<n-1-i) maxindex=n-1-i;
            else if(max<A[n-1-i]) {max=A[n-1-i]; maxindex=n-1-i;}
        }
